...
// assign a temporary unique ID to the order contents to prevent hack attempts during the checkout procedure
    ...
    function add_cart($products_id, $qty = '1', $attributes = '', $notify = true) {
      ...
      $products_options_required = $this->osc_get_po_required($products_id);
      ...
      if (is_array($attributes) && !empty($attributes) ) {          
        ...
        while (list($option, $value) = each($attributes)) {           
          if (!is_numeric($option) || !is_numeric($value) ) {
            ...
            $check_query = osc_db_query("select products_attributes_id from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id = '" . (int)$products_id . "' and options_id = '" . (int)$option . "' and options_values_id = '" . (int)$value . "' or  '" . (int)$value . "' = 0 limit 1");
      ...
      }elseif(osc_has_product_attributes($products_id) && $products_options_required == false) {
        $attributes_pass_check = true; 
      }elseif(osc_has_product_attributes($products_id) && $products_options_required == true){
         $attributes_pass_check = false;  
      }
...
// attributes price
    ...
    // if product has attributes required
    function osc_get_po_required($pid) {
        
    $po_required_query = osc_db_query("select po.products_options_required as required from " . TABLE_PRODUCTS_OPTIONS . " po, " . TABLE_PRODUCTS_ATTRIBUTES . " pa where pa.products_id = '" . $pid . "' and pa.options_id = po.products_options_id limit 1");
    $required = osc_db_fetch_array($po_required_query);
