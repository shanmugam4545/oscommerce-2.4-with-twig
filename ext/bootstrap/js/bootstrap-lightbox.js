...
* ========================================================= */! function($) {
    ...
    var Lightbox = function(element, options) {
        ...
        this.options.remote && this.$element.find('.lightbox-body')
    ...
    Lightbox.prototype = {
        ...
        hide: function(e) {
            ...
            $(document)
                .off('focusin.lightbox');

            this.$element.removeClass('in')
                .attr('aria-hidden', true);

            $.support.transition && this.$element.hasClass('fade') ? this.hideWithTransition() : this.hideLightbox();
        },
        enforceFocus: function() {
            var that = this;
            $(document)
                .on('focusin.lightbox', function(e) {
        ...
        centerImage: function() {
            ...
            if (that.options.resizeToFit) {
                ...
                if ($img.length) {
                    ...
                    var wOffs = 50; // STYLE ?
                    var hOffs = 40; // STYLE ?
                    if (that.$element.find('.lightbox-header')
                        .length > 0) {
                        wOffs += 40;
                        hOffs += 10;
                    }
                    if (that.$element.find('.lightbox-footer')
                        .length > 0) {
                        hOffs += that.$element.find('.lightbox-footer')
                            .height();
                        hOffs += 10;
                    }
                    $img.css('max-width', $(window)
                        .width() - wOffs);
                    $img.css('max-height', $(window)
                        .height() - hOffs);
        ...
        cloneSize: function() // The cloneSize function is only run once, but it helps keep image jumping down
        {
            ...
            that.$clone = that.$element.filter(':first')
                .clone()
                .css({
                'position': 'absolute',
                'top': -2000,
    ...
    $(document)
        .on('click.lightbox.data-api', '[data-toggle="lightbox"]', function(e) {
        var $this = $(this);
        ...
        if (img) {
            $target.data('original-content', $target.find('.lightbox-content')
                .html());
            $target.find('.lightbox-content')
        ...
        $target.lightbox(option)
            ...
            if (img) {
                $target.find('.lightbox-content')
                    .html($target.data('original-content'));
                img = undefined;
            }
        });
    })
